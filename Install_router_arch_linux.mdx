...
```code
# sudo nvim /etc/systemd/network/20-wired-external-dhcp.network
[Match]
...
ip route add default via 198.18.0.1 dev utun table 233
ip route add 198.18.0.0/16 dev utun table 233
ip route add 10.0.0.0/24 dev br_lan table 233

ip rule add from 10.0.0.0/24 table 233
```
